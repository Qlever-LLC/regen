...
export async function uploadFile({
  ...
  const resolvedMime = mimeType || /*contentType(filename) || */'application/octet-stream';

  const bodyStream =
    typeof content === 'string' ? fs.createReadStream(content) :
    content instanceof Readable ? content :
